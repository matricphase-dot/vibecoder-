...
function ProjectItem({ project, onLoad, onJoin, onGitHub, onDownload }) {
  ...
  return (
    ...
    >
      ...
      <div
        className={`flex gap-1 transition-all duration-300 ${hover ? "opacity-100 translate-x-0" : "opacity-0 translate-x-2"}`}
      >
        <button
          onClick={(e) => {
            e.stopPropagation();
...
export default function MainApp() {
  ...
  const addLog = (message) => {
    setLogs((prev) => [
      ...prev,
      { time: new Date().toLocaleTimeString(), text: message },
    ]);
  };

  const updateAgentStatus = (agentName, status) => {
    setAgents((prev) =>
      prev.map((a) => (a.name === agentName ? { ...a, status } : a)),
    );
  };
    ...
    try {
      const res = await fetch("http://localhost:8000/deploy", {
        ...
        body: JSON.stringify({
          ...
          project_name: task.replace(/\s+/g, "-").toLowerCase(),
        }),
      });
      const data = await res.json();
      if (data.url) {
    ...
    try {
      const res = await fetch("http://localhost:8000/github/push", {
        ...
        body: JSON.stringify({
          ...
          repo_name:
            repoName || `vibecoder-${extractProjectId(selectedProject.url)}`,
          private: repoPrivate,
  ...
  return (
    <div
      className={`h-screen flex flex-col ${theme === "dark" ? "bg-[#0a0a0a]" : "bg-gray-50"}`}
    >
      ...
      {/* Header */}
      <header
        className={`relative z-10 border-b ${theme === "dark" ? "border-gray-800/50 bg-gray-900/30" : "border-gray-200/50 bg-white/30"} backdrop-blur-xl p-4 flex items-center justify-between`}
      >
      ...
      <div className="flex-1 flex overflow-hidden relative z-10">
        ...
        <div
          className={`w-72 ${theme === "dark" ? "bg-gray-900/30 border-r border-gray-800/50" : "bg-white/30 border-r border-gray-200/50"} backdrop-blur-xl p-4 overflow-y-auto`}
        >
        ...
        {/* Center - Code editor */}
        <div
          className={`flex-1 flex flex-col ${theme === "dark" ? "bg-gray-800/20" : "bg-gray-50/20"} backdrop-blur-sm`}
        >
          <div
            className={`border-b px-2 flex items-center overflow-x-auto ${theme === "dark" ? "border-gray-800/50" : "border-gray-200/50"}`}
          >
            {Object.keys(files).length > 0 ? (
              Object.keys(files).map((file) => (
                <button
                  ...
                  className={`px-4 py-2 text-sm font-mono border-b-2 transition-all ${
                    activeFile === file
        ...
        {/* Right - Live preview */}
        <div
          className={`w-96 ${theme === "dark" ? "bg-gray-900/30 border-l border-gray-800/50" : "bg-white/30 border-l border-gray-200/50"} backdrop-blur-xl p-4 overflow-hidden flex flex-col`}
        >
      ...
      <div
        className={`h-64 border-t ${theme === "dark" ? "border-gray-800/50 bg-gray-900/30" : "border-gray-200/50 bg-white/30"} backdrop-blur-xl flex flex-col`}
      >
        {/* Agent status */}
        ...
        <div className="flex-1 overflow-y-auto p-3 font-mono text-xs">
          ...
          {logs.map((log, i) => (
            <div
              key={i}
              className="text-gray-400 border-l-2 border-purple-500 pl-2 mb-1"
            >
