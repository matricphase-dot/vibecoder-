...
import { Download, Star, Upload, Filter, Search, MessageCircle, X } from 'lucide-react';
...
const Marketplace = () => {
    ...
    const fetchReviews = async (itemId) => {
        try {
            const res = await fetch(`${import.meta.env.VITE_API_URL}/api/marketplace/items/${itemId}/reviews`);
            const data = await res.json();
            setReviews(data);
        } catch (err) {
    ...
    const submitReview = async (e) => {
        ...
        try {
            ...
            const res = await fetch(`${import.meta.env.VITE_API_URL}/api/marketplace/items/${selectedItem.id}/reviews`, {
                method: 'POST',
                headers: {
    ...
    return (
        <div className="container mx-auto p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {items.map(item => (
                    <div key={item.id} className="border rounded-lg p-4 shadow hover:shadow-lg transition bg-gray-800/30 border-gray-700/30 cursor-pointer" onClick={() => openItem(item)}>
                        ...
                        <div className="flex justify-between items-center mt-4">
                            ...
                            <button
                                onClick={(e) => { e.stopPropagation(); downloadItem(item.id); }}
                ...
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                    <div className="bg-gray-900 p-6 rounded-lg w-96 max-h-[80vh] overflow-y-auto">
                        <div className="flex justify-between items-center mb-4">
                            <h2 className="text-xl font-bold">{selectedItem.name} - Reviews</h2>
                        ...
                        <div className="mb-4 max-h-60 overflow-y-auto">
                            ...
                            {reviews.map(rev => (
                                <div key={rev.id} className="border-b border-gray-700 pb-2 mb-2">
                                    <div className="flex items-center gap-2">
                                        {renderStars(rev.rating)}
                                        <span className="text-sm text-gray-400">{rev.user?.username || 'Anonymous'}</span>
                                    </div>
                                    {rev.comment && <p className="text-sm text-gray-300 mt-1">{rev.comment}</p>}
                        ...
                        {localStorage.getItem('token') && (
                            <form onSubmit={submitReview} className="border-t border-gray-700 pt-4">
